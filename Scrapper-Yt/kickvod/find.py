...
TARGET_SCRIPT = BASE_DIR / "find_most_replayed_chat.py"
RECOVERY_MARKER = (
    '#!/usr/bin/env python3\n"""\n'
...
def _build_interactive_args() -> list[str]:
    print("Find Moment CLI - Interactive")
    vod_url = _prompt_text("Masukkan URL VOD Kick")
    source = _prompt_choice("Pilih source data:", ["kickvod", "auto", "kick"], default_index=0)
    ...
    args = [
        ...
        source,
        "--window-seconds",
        _prompt_text("Window detik", "60"),
        "--top",
        _prompt_text("Top interval", "10"),
        "--max-requests",
        _prompt_text("Max requests", "600"),
        "--kickvod-step-ms",
        _prompt_text("Kickvod step ms", "10000"),
        "--kickvod-timeout",
        _prompt_text("Kickvod timeout (detik)", "45"),
        "--kickvod-retries",
        _prompt_text("Kickvod retries", "5"),
    ]

    if _prompt_yes_no("Batasi ke range waktu tertentu?", default=False):
        args.extend(
            [
                "--range-start",
                _prompt_text("Range start (HH:MM atau HH:MM:SS)", "01:00"),
                "--range-end",
                _prompt_text("Range end (HH:MM atau HH:MM:SS)", "02:00"),
            ]
        )

    args.extend(["--json-output", _prompt_text("Nama file output JSON", "top_moments.json")])
    return args


def _recover_target_script() -> bool:
    source_text = Path(__file__).read_text(encoding="utf-8")
    start = source_text.find(RECOVERY_MARKER)
    if start == -1:
        return False

    recovered = source_text[start:]
    trailer = "\n'''"
    if recovered.endswith(trailer):
        recovered = recovered[: -len(trailer)] + "\n"
    else:
        cut = recovered.rfind(trailer)
        if cut != -1:
            recovered = recovered[:cut] + "\n"

    TARGET_SCRIPT.write_text(recovered, encoding="utf-8")
    return TARGET_SCRIPT.exists()


def main() -> int:
    if not TARGET_SCRIPT.exists():
        if not _recover_target_script():
            print(f"Error: target script tidak ditemukan: {TARGET_SCRIPT}", file=sys.stderr)
            return 1

    forward_args = _build_interactive_args() if len(sys.argv) == 1 else sys.argv[1:]
    command = [sys.executable, str(TARGET_SCRIPT), *forward_args]
...
#!/usr/bin/env python3
"""
Interactive launcher for find_most_replayed_chat.py.
...
def _build_interactive_args() -> list[str]:
    ...
    window_seconds = _prompt_text("Window detik", "60")
    top = _prompt_text("Top interval", "10")
    max_requests = _prompt_text("Max requests", "600")
    step_ms = _prompt_text("Kickvod step ms", "10000")
    timeout = _prompt_text("Kickvod timeout (detik)", "45")
    retries = _prompt_text("Kickvod retries", "5")
    ...
    use_range = _prompt_yes_no("Batasi ke range waktu tertentu?", default=False)
    if use_range:
        range_start = _prompt_text("Range start (HH:MM atau HH:MM:SS)", "01:00")
        range_end = _prompt_text("Range end (HH:MM atau HH:MM:SS)", "02:00")
        args.extend(["--range-start", range_start, "--range-end", range_end])

    output_json = _prompt_text("Nama file output JSON", "top_moments.json")
    args.extend(["--json-output", output_json])
    return args
...
def main() -> int:
    ...
    if len(sys.argv) == 1:
        forward_args = _build_interactive_args()
    else:
...
def main() -> int:
    ...
    if len(sys.argv) == 1:
        ...
        # Passthrough mode if user still wants manual arguments.
        forward_args = sys.argv[1:]
